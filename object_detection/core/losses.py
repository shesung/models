...
class RBoxLocalizationLoss(Loss):
  def __init__(self, alpha):
    self._alpha = alpha
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    if len(prediction_tensors) != 2 or len(target_tensors) != 2:
      raise ValueError('length of prediction_tensors and target_tensors must be 2')
    ...
    pre_angle = tf.reshape(prediction_tensor[:,:,4], [-1])
