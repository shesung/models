...
class EastIOULocalizationLoss(Loss):
	...
	def _compute_loss(self, prediction_tensor, target_tensor, weights):
		...
		pre_angle = tf.reshape(prediction_tensor[:,:,4], [-1, 1])
		target_angle = tf.reshape(target_tensor[:,:,4], [-1, 1])
		angle_loss = self._alpha * (1.0 - tf.cos(pre_angle - target_angle))
		return tf.reduce_sum(tf.reshape(weights, [-1]) * iou_loss) + tf.reduce_sum(tf.reshape(weights, [-1]) * angle_loss)
'''
...
class EastMaskLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    return tf.reduce_sum(per_entry_cross_ent * tf.reshape(weights, [-1]))
